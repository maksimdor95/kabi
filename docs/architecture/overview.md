...
```
┌───────────────────────────────────────────────┐
│  Интерфейс:  bot/ (aiogram) · app/api/ + web/   │  тонкие слои, без бизнес-логики
│              Telegram-чат        Mini App       │
└───────────────────────┬─────────────────────────┘
                        ▼
┌───────────────────────────────────────────────┐
...
| `api` + `web` | интерфейс | Mini App: auth по `initData`, HTTP-контракт, статика | M11 | [miniapp.md](../services/miniapp.md) |
...
**Очередь сейчас:** M7–M11 и MU-A закрыты. Дальше — проактивный пинг и источники.

| Этап | Фокус | Статус |
|------|--------|--------|
| **M7** | Новые источники вакансий (Хабр, Getmatch, TG, career API-волна A) | ✅ |
...
| **M11** | Telegram Mini App — `docs/services/miniapp.md` | ✅ |

**M5** = монитор известных URL. **M6** = discovery (PaperCall → фильтр регион/ниша →
Opportunity). Yaml только bootstrap.

Вне scope (уровень B): SaaS-тенанты, биллинг, success fee, данные о третьих лицах.
N Telegram-профилей в одном боте — MU-A, см. `docs/services/multiuser.md`.
